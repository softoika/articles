...
Subject/BehaviorSubjectを使うことでインメモリキャッシュやシンプルなデータストアを実装することができます。
以下は2回目以降の呼び出しではインメモリキャッシュを使う実装例です。
```ts
...
class UserProfileComponent implements OnInit {
  ...
  // ユーザープロフィールにフォロワーのリンクがあってマウスオーバーしたときのイメージ
  onMouseOver(userId: User['id']) {
